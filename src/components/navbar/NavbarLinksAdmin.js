  ...
  export default function HeaderLinks(props) {
	...
	const logOut = async (message) => {
	  ...
	  try {
		if (isSignedIn) {
		  ...
		  console.log("Logging out via custom logic...");
		  localStorage.clear();
		  sessionStorage.clear();
		  dispatch({ type: "LOGOUT" }); // Reset user state in Redux
		  if (message) {
			toast.error(message);
		  } else {
	...
	return (
	  ...
	  >
		...
		{/* Custom Avatar Menu for Non-Clerk Users */}
		{!isSignedIn && (
			...
			>
			  ...
			  <Flex flexDirection="column" p="10px">
				...
				<MenuItem
				  ...
				  borderRadius="8px"
				  px="14px"
				  isDisabled={isLoggingOut} // Disable button during logout
				>
				  <Text fontSize="sm">{isLoggingOut ? "Logging out..." : "Log out"}</Text>
				</MenuItem>
			  </Flex>
			</MenuList>
		  </Menu>
